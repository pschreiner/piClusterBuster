...
if( length(readLines(xls))>1 ) {  
	...
	{
		query<-append(query, as.character(seqnames(gr)[ii])); subject<-append(subject,"picluster"); perc_id<-append(perc_id,as.character(filt)); len<-append(len,width(gr)[ii]); mis<-append(mis,"."); gap<-append(gap,"."); qstart<-append(qstart,"."); qend<-append(qend,"."); meta1<-append(meta1,elementMetadata(gr)[[1]][ii]); bit<-append(bit,"CENSOR") 
